...
import com.ypyg.shopmanager.activity.customer.FragmentCustomerSingle;
...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private final PageListener2 pageListener2 = new PageListener2();
	private final PageListener3 pageListener3 = new PageListener3();
	...
	/** 管理FragmentOrderList **/
	private FragmentOrderListSingle[] fListSingles;
	/** 管理FragmentCustomerSingle **/
	private FragmentCustomerSingle[] customerSingles;

	public void setViewPager2(ViewPager pager, FragmentOrderListSingle[] fListSingles) {
		this.pager = pager;
		this.fListSingles = fListSingles;
		if (pager.getAdapter() == null) {
			throw new IllegalStateException("ViewPager does not have adapter instance.");
		}

		pager.setOnPageChangeListener(pageListener2);

		notifyDataSetChanged();
	}

	public void setViewPager3(ViewPager paramViewPager, FragmentCustomerSingle[] paramArrayOfFragmentCustomerSingle) {
		this.pager = paramViewPager;
		this.customerSingles = paramArrayOfFragmentCustomerSingle;
		if (paramViewPager.getAdapter() == null)
			throw new IllegalStateException("ViewPager does not have adapter instance.");
		paramViewPager.setOnPageChangeListener(pageListener3);
		notifyDataSetChanged();
	}
	...
	private class PageListener2 implements OnPageChangeListener {
		...
		@Override
		public void onPageSelected(int position) {
			// 重置所有offset,关闭其它页面请求，除了当前页面
			for (int i = 0; i < fListSingles.length; i++) {
//				fListSingles[i].offset = 0L;
//				if (i == position) {
//					fListSingles[i].isRequest = true;
//				} else {
//					fListSingles[i].isRequest = false;
//				}
			}

			// 主动刷新列表
//			fListSingles[position].setReflash();
			if (delegatePageListener != null) {
				delegatePageListener.onPageSelected(position);
			}
		}
	...
	private class PageListener3 implements ViewPager.OnPageChangeListener {
		@Override
		public void onPageScrollStateChanged(int state) {
			if (state == ViewPager.SCROLL_STATE_IDLE) {
				scrollToChild(pager.getCurrentItem(), 0);
		...
		public void onPageSelected(int position) {
			// 重置所有offset,关闭其它页面请求，除了当前页面
			for (int i = 0; i < customerSingles.length; i++) {
				customerSingles[i].offset = 0L;
				if (i == position) {
					customerSingles[i].isRequest = true;
				} else {
					customerSingles[i].isRequest = false;
				}
			}

			// 主动刷新列表
			customerSingles[position].setReflash();
			if (delegatePageListener != null) {
				delegatePageListener.onPageSelected(position);
			}
