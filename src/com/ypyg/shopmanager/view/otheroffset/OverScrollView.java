package com.ypyg.shopmanager.view.otheroffset;
/*
...
public class OverScrollView extends FrameLayout implements OnTouchListener {
	...
	static final float OVERSHOOT_TENSION = 0.5f;
		...
		switch (action & MotionEvent.ACTION_MASK) {
			...
			case MotionEvent.ACTION_MOVE :
				if (mIsBeingDragged) {
					...
					if (isOverScrolled()) {
						// when overscrolling, move the scroller just half of
						// the
	...
	public boolean isOverScrolled() {
		return (getScrollY() < child.getPaddingTop() || getScrollY() > child
				.getBottom() - child.getPaddingBottom() - getHeight());
	}
	...
	public final void smoothScrollToBottom() {
		smoothScrollTo(0, child.getHeight() - child.getPaddingTop()
				- getHeight());
	}
	...
	protected boolean onRequestFocusInDescendants(int direction,
		...
		final View nextFocus = previouslyFocusedRect == null
				? FocusFinder.getInstance()
						...
						.getInstance().findNextFocusFromRect(this,
	...
	@Override
	protected void onScrollChanged(int leftOfVisibleView, int topOfVisibleView,
			int oldLeftOfVisibleView, int oldTopOfVisibleView) {
		...
		if (isInFlingMode) {
			...
			if (topOfVisibleView < paddingTop
					|| topOfVisibleView > contentBottom - displayHeight) {
				if (topOfVisibleView < paddingTop) {
					mScroller.startScroll(0, topOfVisibleView, 0, paddingTop
							- topOfVisibleView, 1000);
				} else if (topOfVisibleView > contentBottom - displayHeight) {
					mScroller.startScroll(0, topOfVisibleView, 0, contentBottom
							- displayHeight - topOfVisibleView, 1000);
				}
		...
		super.onScrollChanged(leftOfVisibleView, topOfVisibleView,
				oldLeftOfVisibleView, oldTopOfVisibleView);
	}
	...
	public void fling(int velocityY) {
		if (getChildCount() > 0) {
			...
			View newFocused = findFocusableViewInMyBounds(movingDown,
			...
			if (newFocused != findFocus()
					&& newFocused.requestFocus(movingDown
							? View.FOCUS_DOWN
							: View.FOCUS_UP)) {
				mScrollViewMovedFocus = true;
				mScrollViewMovedFocus = false;
	...
	private boolean overScrollView() {
		...
		if (currScrollY < contentTop) {
			...
			// Scroll to content top
			if (child.getHeight() - child.getPaddingTop()
					- child.getPaddingBottom() < displayHeight) {
