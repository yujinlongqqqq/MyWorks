...
public class CropImage {
	...
	public static final File FILE_SDCARD = new File(Constants.FILE_SDCARD);

	public static final File FILE_LOCAL = new File(FILE_SDCARD, "smallhead");
	...
	private Bitmap onSaveClicked(Bitmap bm) {
		...
		int width = (int) mContext.getResources().getDimension(
				R.dimen.crop_image);// dr.width(); // modify by yc
		int height = (int) mContext.getResources().getDimension(
				R.dimen.crop_image);// dr.height();
		Bitmap croppedImage = Bitmap.createBitmap(width, height,
				Bitmap.Config.RGB_565);
		{
			Canvas canvas = new Canvas(croppedImage);
