...
import com.ypyg.shopmanager.thread.ThreadPool.BlockingStack;
import com.ypyg.shopmanager.thread.ThreadPool.CometThreadFactory;
...
public class ThreadExecutor extends AbstractExecutorService {
	...
	private final ArrayList<Worker> mWorkers = new ArrayList<Worker>();
	...
	private Runnable getTask() {
		...
		retry : for (;;) {
			...
			try {
				Runnable r = timed ? mWorkQueue.poll(keepAliveTime,
						TimeUnit.NANOSECONDS) : mWorkQueue.take();
				if (r != null) {
					return r;
				}
				timedOut = true;
	...
	public ThreadExecutor(int corePoolSize, int maximumPoolSize,
			...
			RejectedTaskHandler handler) {
		if (corePoolSize < 0 || maximumPoolSize <= 0
				|| maximumPoolSize < corePoolSize || keepAliveTime < 0) {
			throw new IllegalArgumentException();
		}
		mWorkQueue = workQueue == null
				? new BlockingStack<Runnable>()
				: workQueue;
		mThreadFactory = threadFactory == null ? new CometThreadFactory(
				Thread.NORM_PRIORITY) : threadFactory;
	...
	public List<Runnable> emptyOneCoreThread() {
		if (workerCountOf(mCtl.get()) == corePoolSize) {
			return interruptIdleWorkers(1);
		}
		return null;
	}
	...
	public static class DefaultAbortPolicy implements RejectedTaskHandler {
		...
		public void rejectedExecution(Runnable r, ThreadExecutor e) {
			...
			String message = "pool=" + e.getPoolSize() + "/"
					+ e.maximumPoolSize + ", queue=" + queueSize;
