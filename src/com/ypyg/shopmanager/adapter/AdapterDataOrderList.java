...
public class AdapterDataOrderList extends BaseAdapter {
	...
	private TextView data_paymentorder = null;
	private TextView data_shipmentorder = null;

	public AdapterDataOrderList(Context context, String imageTag, List<GoodInfoBean> list) {
		mContext = context;
		mList = list;
	...
	public void setDataTopView(View topView) {
		...
		data_paymentorder = (TextView) dataTopView.findViewById(R.id.data_paymentorder);
		data_shipmentorder = (TextView) dataTopView.findViewById(R.id.data_shipmentorder);
		// 树状图test 之后记得删掉该代码
		// mHistogramView.start(ActivityDataManager.flag2);
		ActivityDataManager.flag2 = 3;
	}

	public void setData(DataOrderCountinfobean bean) {
		if (AppUtil.isNull(bean))
			return;

		if (!AppUtil.isNull(bean.getPaymentorder()))
			data_paymentorder.setText(bean.getPaymentorder());
		if (!AppUtil.isNull(bean.getShipmentorder())) {
			data_shipmentorder.setText(bean.getShipmentorder());
		}
		// mHistogramView.start(ActivityDataManager.flag2);
		ActivityDataManager.flag2 = 3;
	}
