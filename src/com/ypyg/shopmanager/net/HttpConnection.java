...
import com.ypyg.shopmanager.bean.CommenReqBean;
import com.ypyg.shopmanager.common.DataCener;
import com.ypyg.shopmanager.common.httpconnection.RSACryptUtil;
import com.ypyg.shopmanager.thread.HttpThread;
import com.ypyg.shopmanager.util.Log_;

public class HttpConnection implements IHttpService {
	...
	private final DefaultHttpRequestRetryHandler retryHandler = new DefaultHttpRequestRetryHandler(
			0, false);
	 ...
	 */
	public HttpConnection(Context aContext, INetState aState,
			Integer aMaxReadTimeout) {
		this.mContext = aContext;
		this.mNetState = aState;
		this.mMaxReadTimeout = aMaxReadTimeout;
		this.mHttpParams = buildHttpParams(DEFAULT_MAX_PER_ROUTE,
				mMaxReadTimeout);
	}

	private HttpParams buildHttpParams(Integer aMaxPerRoute,
			Integer aMaxReadTimeout) {
		...
		Scheme aHttp = new Scheme("http",
				PlainSocketFactory.getSocketFactory(), DEFAULT_HTTP_PORT);
		registry.register(aHttp);
		Scheme aHttps = new Scheme("https",
				SSLSocketFactory.getSocketFactory(), DEFAULT_HTTPS_PORT);
	...
	private void checkUniwapAccessing(Exception e) {
		synchronized (lock) {
			if (!mNetState.isUniWap()) {
				...
				HttpHost host = new HttpHost(DEFAULT_CMWAP_HOST,
						DEFAULT_HTTP_PORT);
		...
		if (response != null && response.getStatusLine() != null) {
			...
			if (null != aEntity) {
				if (aEntity.isChunked()) {
					...
					return (((iStatusCode == HttpStatus.SC_OK) || (iStatusCode == HttpStatus.SC_PARTIAL_CONTENT))
							&& (lContentLen > 0) && (lContentLen <= Integer.MAX_VALUE));
				}
			}
		}
		return false;
	...
	private HttpEntity encryption(IReq aReq, HttpEntity aEntry)
			throws Exception {
		DataCener mCener = DataCener.getInstance();
		CommenReqBean reqBean = new CommenReqBean();
		String data = EntityUtils.toString(aReq.getEntry());
		String key;
		try {
			reqBean.setDatamode("1");
			key = CryptUtil.initKey();// 动态生成 DES key
			// 加密报文
			reqBean.setData(CryptUtil.encryptBASE64(CryptUtil.encrypt(
					data.getBytes("UTF-8"), key.getBytes("UTF-8"))));
			// 密钥版本号
			reqBean.setKeyvernum(mCener.getmKeyvernum());
			// 通过公钥 加密 DES key
			reqBean.setDecryptkey(CryptUtil.encryptBASE64(RSACryptUtil
					.encryptByPublicKey(key.getBytes(), mCener.getmPublicKey())));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return reqBean.getEntry();
	}

	public HttpResponse getHttpResp(IReq aReq, HttpEntity aEntry)
			throws Exception {
		...
		while (tryTimes < mRetryMax) {
			try {
				...
				String sUrl = aReq.getUrl();// 需要更换代码 以后地址不会变
				if (null == sUrl)
					break;
				// new connection encryption
				try {
					Log_.e("req", EntityUtils.toString(aEntry));
				} catch (Exception e) {
				}
				HttpEntity newEntry = encryption(aReq, aEntry);
				HttpResponse aResp = getHttpRespException(sUrl, newEntry,
						httpThread);
				if (null != aResp)
					// new connection decryption
					return aResp;
				tryTimes++;
			} catch (Exception e) {
				e.printStackTrace();
	...
	private HttpResponse getHttpRespException(String aUrl, HttpEntity aEntry,
			HttpThread aHttpThread) throws ClientProtocolException, IOException {
		...
		if (aResp != null) {
			...
			if (null != stLine) {
				...
				if (iCode == HttpStatus.SC_NOT_FOUND
						|| iCode == HttpStatus.SC_INTERNAL_SERVER_ERROR) {
	 ...
	 */
	@Override
	public HttpResponse getHttpRespWithDoGet(String aUrl, long begin, long end)
			throws Exception {
		HttpThread httpThread = (HttpThread) Thread.currentThread();
		try {
			...
			httpThread.mDefaultPost.addHeader("Range", "bytes=" + begin + "-"
					+ end);
			// 发起请求
			HttpResponse aResp = httpThread.defaultClient
					.execute(httpThread.mDefaultPost);
			if (aResp != null) {
				StatusLine stLine = aResp.getStatusLine();
				if (null != stLine) {
					int iCode = stLine.getStatusCode();
			...
			if (isSuccess(aResp)) {
				...
				if (httpThread.mDefaultPost != null
						&& (!httpThread.mDefaultPost.isAborted())) {
